...
<html>
    <head>
        ...
        <title>ΕΛΕΓΧΟΣ ΠΑΡΑΛΑΒΗΣ ΕΝΔΟΔΙΑΚΙΝΙΣΗΣ</title>
    ...
    <center>
        ...
        <h3>ΔΕΛΤΙΟ ΠΑΡΑΛΑΒΗΣ:${endoBinder.endoParalavis.number} </h3>
        <hr>
        ...
        <table>
            ...
            <tbody id="tableBody">
                <%
                    int x = 1;
                    EndoBinder endoBinder = (EndoBinder) request.getAttribute("endoBinder");
                    EndoParalavis endoParalavis = endoBinder.getEndoParalavis();
                    LinkedHashMap<String, Item> deliveredItems = endoParalavis.getItems();
                    LinkedHashMap<String, Double> sentItems = endoBinder.getTotalSentItems();
                    for (Map.Entry<String, Item> deliveredItemEntry : deliveredItems.entrySet()) {
                        Item item = deliveredItemEntry.getValue();
                        Double sentQuantity = null;
                        if (sentItems.containsKey(item.getCode())) {
                            sentQuantity = sentItems.remove(item.getCode());
                        }
                        out.println("<tr>");
                        out.println("<td>");
                        out.println(x);
                        ...
                        out.println("<td>");
                        out.println("<input  class='sent' type='number' id='" + item.getCode() + "@sent' value='" + sentQuantity != null ? sentQuantity : 0 + "' readonly width='10px'>");
                        out.println("</td>");

                        out.println("<td>");
                        out.println("<input class='delivered' type='number' id='" + item.getCode() + "@delivered' value='" + item.getQuantity() + "'>");
                        out.println("</td>");
